...
public interface AuthMapper {
    ...
    @Select("""
        SELECT student_id AS userId, name, 'STUDENT' AS role, NULL AS password FROM Student WHERE student_id = #{userId}
        UNION
        SELECT prof_id AS userId, name, 'PROFESSOR' AS role, password FROM Professor WHERE prof_id = #{userId}
        UNION
        SELECT admin_id AS userId, NULL AS name, 'ADMIN' AS role, NULL AS password FROM Admin WHERE admin_id = #{userId}
        """)
    @Results({
        @Result(column = "userId", property = "userId"),
        @Result(column = "name", property = "name"),
